---
name: route-analogy-miner
description: Use to generate and compare physical metaphor routes with proof birth, wow asset events, and residue without selecting the route for the human.
---
...
1. Generate routes that differ in mechanism, not just color or layout.
2. For each route, state: object does X to constraint Y, causing proof Z.
3. Attach chart/source proof to the event when proof is critical.
4. Score no-label clarity, Family B/C fit, implementation feasibility, and copy risk.
5. Do not select the route; package it for human decision.
